...
router.put('/:id/pay', isAuth, async (req, res) => {
  ...
  if (order) {
    ...
    res.send({ msg: 'Order successfully paid', order: updateOrder });
  } else {
...
router.post('/confirmOrderEmail', isAuth, async (req, res) => {
  ...
  try {
    ...
    const { response } = await transport.sendMail({
      to: req.user.email,
      subject: 'YetiShop Order-Success-Email',
      html: `
            
            <h2>YetiShop-shopping</h2>
            <p>thanks for shopping to YetiShop</p>
            <ul>
        ...
        if (response) {
            console.log('working...');
            return res.send({
            msg: 'Order confirm email has been sent successfully',
